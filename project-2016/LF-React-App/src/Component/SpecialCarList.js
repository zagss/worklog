...
export default class SpecialCarList extends Component{
  ...
  componentDidMount(){
    ...
    this.connectWebViewJavascriptBridge(bridge =>{
       ...
       bridge.init((message, responseCallback) =>{
        ...
        //接收安卓发来的消息   并返回给安卓通知
       bridge.registerHandler("SpecialCarListPage", (data, responseCallback) =>{
  ...
  handleScroll(){
    ...
    if(reachBottom()){
      console.log('滚动到底了')
      // fetch("http://192.168.1.202:8010/api/v1/CarProMidd?filter[id_city]=9EAD4557-A2A4-7599-D77B-36BCA7478F5B&filter[car_type]=2").then(response => {
      //   return response.json();
      // }).then(data => {
      //   this.setState({
      //     yachtList: this.state.yachtList.concat(data.data)
...
class List extends Component{
  ...
  jumpPage(id,res){
    ...
    this.props.connectWebViewJavascriptBridge(bridge =>{
      bridge.callHandler('intent', {'action':'OrderSpecialCar','data': {
          'ID':id,
        ...
        {this.props.yachtList.map((res,index) =>{
          ...
          return(
            <div  className="scl_content" key={index}>
              ...
              <div onClick={this.jumpPage.bind(this,res.id,res)} style={{height: '18.99rem',overflow:'hidden'}}>
              <img src={imgUrl1} />
              </div>
                <p className="gold">
                <small>{res.attributes.day_price}</small>
                <small>{res.attributes.hour_price}</small>
                </p>
