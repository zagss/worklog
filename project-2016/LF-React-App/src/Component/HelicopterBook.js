...
componentDidMount(){
  ...
  this.connectWebViewJavascriptBridge(bridge =>{
      ...
      bridge.registerHandler("HelicopterBookPage", (data, responseCallback) =>{
        // alert(data);
             self.setState({
  ...
  bookQuick(){
    ...
    const data_total = {
      ...
      'total_price': this.state.dataTotal.fly_price * parseFloat(this.refs.hour.value),
  ...
  upperCase(){
    this.setState({
      moneyTotal: this.state.dataTotal.fly_price * parseFloat(this.refs.hour.value)
    })
  }
      ...
      <div style={{background: '#f3f4f5',position: 'relative'}}>
        <div className="hb-pic">
          ...
          <div className="hb-blur">
            ...
            <small>￥{this.state.dataTotal.fly_price}/小时</small>
          </div>
        </div>
        <div className="hb-seat">
          <big>
            <small>座位数: {this.state.dataTotal.fly_seat}</small>
          </big>
          <big>
            <small>外观: {this.state.dataTotal.fly_color}</small>
          </big>
        </div>
        <div  className="hb-form" style={{height: '39rem',overFlow: 'hidden'}}>
          <form>
            ...
            <h3>观光时长 <input ref="hour" type="tel" onInput={this.upperCase.bind(this)} maxLength='3'/> </h3>
        ...
        <div className="pay-quick"  style={{paddingLeft: '1.5rem',paddingRight: '1.5rem',position: 'fixed',bottom: '0'}}>
          <p>预付金：<span style={{display:'inline-block',width: '2rem',overFlow: 'hidden'}}>￥1000</span></p>
