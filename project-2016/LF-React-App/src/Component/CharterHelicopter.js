...
export default class CharterHelicopter extends Component {
  constructor(props){
    super(props);
    this.state={
      data:[],
    ...
    //     })
        ...
        this.connectWebViewJavascriptBridge(bridge =>{
           ...
           bridge.init((message, responseCallback) =>{
            ...
            //接收安卓发来的消息   并返回给安卓通知
           bridge.registerHandler("CharterHelicopterPage", (data, responseCallback) =>{
          // alert(data)
             let responseData = "haoqia";
             responseCallback(responseData);
                self.setState({
                  data: JSON.parse(data).data,
                  datainc: JSON.parse(data).included
                })
           });
       });
  }
  jumpNextPage(id,item){
    ...
    let _this = this;
    this.connectWebViewJavascriptBridge(bridge =>{
      bridge.callHandler('intent', {'action':'HelicopterBook','data': {
          'ID':id,
    ...
    let heliList = this.state.data.map((res,index) =>{
      ...
      return(
        <div className="lt-pic" onClick={this.jumpNextPage.bind(this,res.id,res)} style={{position: 'relative'}}>
          ...
          <p style={{fontSize: '16px'}}><b>{res.attributes.fly_intro}</b> <i style={styleI}>{res.attributes.fly_seat}位</i><span>￥{res.attributes.fly_price}<small>/小时</small></span></p>
        </div>
      );
    })
    return(
