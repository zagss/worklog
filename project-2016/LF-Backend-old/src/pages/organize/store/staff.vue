        ...
        <div class='table-chunk'>
          ...
          <div class='table-body'>
              <el-table :data='tableData' v-loading="loading">
                  ...
                  <el-table-column prop="user.name" label="姓名" width="100"></el-table-column>
                  <el-table-column prop="id_number" label="身份证" width="180"></el-table-column>
                  <el-table-column prop="user.mobile" label="联系电话" width="130"></el-table-column>
                  <el-table-column prop="user.username" label="账号" width="180"></el-table-column>
                  <el-table-column prop="user.role_id" label="角色" width="100">
              ...
              <el-form :model="newForm" :inline="true">
                <el-form-item v-for="(val, key) in labelData.user" :key='key' :label="val.label" label-width="120px">
                   <el-select v-if="key === 'role_id'" v-model="newForm.role_id" placeholder="请选择">
                    ...
                    <el-radio-group v-model="newForm.is_push" v-else-if="key === 'is_push'" @change="handleChange">
...
<script>
  ...
  import { staffRole } from '../../../config/role'
  export default {
    data () {
      return {
        scope: {},
    ...
    methods: {
      fetchData (start = 1) {
        var res = getStaff(this.$route.query.pid, start)
        const that = this
        res.then(res => {
          that.loading = false
